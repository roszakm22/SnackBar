...
export default function DashboardClient({ displayName }: { displayName: string }) {
  ...
  const outlook = useMemo(() => {
    ...
    const nonSalesDepositsCents = data.cardAudit.adjustments.reduce((sum, row) => sum + row.amountCents, 0);
  ...
  }, [data.cardAudit.adjustments, data.forecastSettings, data.ledger, data.openingCardBalanceCents]);
      ...
      <main className="workspace">
        <Tabs defaultValue="review">
          ...
          <TabsContent value="outlooks" className="section-stack">
            ...
            <section className="outlook-summary"><div><span className="eyebrow light">CURRENT OPERATING BALANCE</span><strong>{money(outlook.currentBalanceCents)}</strong><p>Starting card funds + approved income + non-sales deposits − recorded expenses</p></div><div className="pace-callout"><span>Current revenue pace</span><b className="positive">{money(outlook.dailyRevenueCents)}/day</b><em>{outlook.confidence} · {outlook.operatingDays} of 14 days</em></div></section>
